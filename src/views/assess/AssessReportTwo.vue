    ...
    <div class="container" v-if="!show">
      ...
      <div class="tableWraper">
        <div class="tableCon">
          <table border="1">
            <tr>
              <th colspan="2" style="line-height:0.4rem;">{{report.title}}</th>
            </tr>
            <tr>
            ...
            <template v-if="fail">
              ...
              <td colspan="2">
                <p style="text-indent: 2em">
                  ...
                  <div class="scrollWrap" v-if="report.Object.zhuanli.length != 0">
                  <p style="font-weight:700;">发明专利</p>
                  <table rules="all">
                    <tbody border="1">
                      <tr v-for="(item, idx) of report.Object.zhuanli" :key="idx">
                        <td>{{item.name}}</td>
                        <td>{{item.time}}</td>
                      </tr>
                    </tbody>
                ...
                <div class="scrollWrap" v-if="report.Object.ruanzhu.length != 0">
                  <p style="font-weight:700;">软件著作权</p>
                  <table rules="all">
                    <tbody border="1">
                      <tr v-for="(item, idx) of report.Object.ruanzhu" :key="idx">
                        <td>{{item.name}}</td>
                        <td>{{item.time}}</td>
                      </tr>
                    </tbody>
                ...
                <p
                  style="font-size: 0.18rem; color: #ababab; margin-top: 0.2rem"
                >
            ...
            <template v-else>
              <tr>
                <td colspan="2">
                  <p style="text-indent:2em;">尊敬的{{report.companyName}}，根据国家《高新技术企业认定管理办法》的评分标准，结合权威机构提供的大数据，系统对贵公司的相关数据进行了分析并作出评估，评估结果如下，敬请参考：</p>
      ...
      <!-- <van-button v-if="fail" style="width: 5.8rem;margin: 0.2rem 0.25rem 0.4rem;" color="#d53c3e" @click="toServe" round>开始沟通服务机构</van-button> -->
    </div>
  </div>
</template>

<script>
import { myAssessInfoTwo } from "@/api/request.js";
...
export default {
  ...
  methods: {
    ...
    getAssessment() {
      myAssessInfoTwo(this.$route.query.assessment).then(res=>{
